  ...
  *)
  ...
  let malloc_with_gc s m e c k =
	...
	(* TODO : Add the code that marks the reachable locations.
       * let _ = ... 
       *)
	let rec evaluelist_to_loclist : (evalue list) -> (loc list) = fun el_in ->
		match el_in with
		| el_h :: el_t -> 
			(
			match el_h with
			| Loc el_h_l -> el_h_l :: evaluelist_to_loclist (el_t)
			| Proc (_,_,el_h_p_env) -> (evaluelist_to_loclist (snd (List.split el_h_p_env))) @ evaluelist_to_loclist (el_t) 
			) 
		| [] -> []
	in
	let rec reachable_locs_consider_record : (loc list * memory ) -> (loc list) = fun (rlcr_ll_in, mem_in) ->
		match rlcr_ll_in with
		| rlcr_h::rlcr_t -> 
			(
			let load_from_mem = (load rlcr_h mem_in) in
			match load_from_mem with
			| R record_in -> (snd(List.split record_in)) @ reachable_locs_consider_record (rlcr_t , mem_in)
			| _ -> reachable_locs_consider_record (rlcr_t, mem_in)
			)
		| [] -> []
	in
	let rec reachable_locs_consider_samebase : (loc list * memory) -> (loc list) = fun (rlcs_ll_in,mem_in) ->
		match rlcs_ll_in with
		| (rlcs_a,rlcs_b) :: rlcs_t ->
			(
			let matcher : (loc * value) -> bool = fun (foo,bar) ->
				if ((fst foo) = rlcs_a) then true
				else false
			in
			fst (List.split (List.find_all matcher mem_in )) @ reachable_locs_consider_samebase (rlcs_t ,mem_in) 
			)
		| [] -> []
	in
	let env_to_reachable_locs : environment -> (loc list) = fun (e2rl_env) ->
		let rl = evaluelist_to_loclist (snd (List.split e2rl_env)) in
		let rl2 = reachable_locs_consider_record (rl,m ) @ rl in
		reachable_locs_consider_samebase (rl2,m) @ rl2
	in
	let rec conti_to_reachable_locs : continuation -> (loc list) = fun (c2rl_conti) ->
		match c2rl_conti with
		| c2rl_conti_head :: c2rl_conti_tail -> 
			(env_to_reachable_locs (snd c2rl_conti_head)) @ ( conti_to_reachable_locs  (c2rl_conti_tail)) 
		| [] -> []
	in
	let _ = reachable_locs :=
		let appa = env_to_reachable_locs (e) in
		let afaik = (conti_to_reachable_locs k) @ (env_to_reachable_locs (e))  in
		afaik
	in 
